...
import { fetchMessages } from "@/lib/api/chatApi";
...
export const SocketProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    newSocket.on("receiveMessage", (message: any) => {
        console.log('heyeyeyeyyy',message);
        ...
        console.log(formattedMessage,"formated");
        
      setMessages((prevMessages) => [...prevMessages, formattedMessage]);

    });

    newSocket.on("updateUsers", (userList: { userId: string; username: string }[]) => {
      const updatedUsers = userList.map((users) => ({
        id: users.userId,
  ...
  return (
    <SocketContext.Provider value={{ socket, messages, connectedUsers, sendMessage }}>
      {children}
